...
# Snakemake rules
####################################################
_ref_settings      = config.get("pipeline", {}).get("reference_module", {}).get("mapping", {}).get("settings", {})
...
if _ref_mapper == "minimap2":
    rule map_reads_to_reference_minimap2:
        input:
            ...
            target="{species}/input/ref/{reference}.mmi",
        output:
    ...
    rule align_reads_to_reference_bwa_aln:
        input:
            ...
            idx=multiext("{species}/input/ref/{reference}.fa", ".amb", ".ann", ".bwt", ".pac", ".sa"),
        output:
    ...
    rule map_reads_to_reference_bwa_mem2:
        input:
            ...
            idx=multiext("{species}/input/ref/{reference}.fa", ".0123", ".amb", ".ann", ".bwt.2bit.64", ".pac"),
        output:
            temp("{species}/processed/reference_module/{reference}/mapped/{individual}_{reference}_unsorted.bam")
        log:
            "{species}/processed/reference_module/{reference}/mapped/{individual}_{reference}.bam.log"
...
rule sort_mapped_reads_bam:
    ...
    output:
        temp("{species}/processed/reference_module/{reference}/mapped/{individual}_{reference}_sorted.bam")
...
rule index_mapped_sorted_reads_bam:
    ...
    output:
        temp("{species}/processed/reference_module/{reference}/mapped/{individual}_{reference}_sorted.bam.bai")
