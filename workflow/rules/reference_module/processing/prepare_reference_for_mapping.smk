...
rule standardize_reference_extension_to_fa:
    output:
        fa="{species}/input/ref/{reference}.fa"
    message:
...
if _ref_mapper == "minimap2":
    rule index_reference_for_mapping_minimap2:
        input:
            target="{species}/input/ref/{reference}.fa"
        output:
            "{species}/input/ref/{reference}.mmi",
    ...
    rule index_reference_for_mapping_bwa_aln:
        input:
            "{species}/input/ref/{reference}.fa"
        output:
            multiext("{species}/input/ref/{reference}.fa", ".amb", ".ann", ".bwt", ".pac", ".sa"),
    ...
    rule index_reference_for_mapping_bwa_mem2:
        input:
            "{species}/input/ref/{reference}.fa"
        output:
            multiext("{species}/input/ref/{reference}.fa", ".0123", ".amb", ".ann", ".bwt.2bit.64", ".pac"),
...
rule index_reference_with_samtools:
    input:
       "{species}/input/ref/{reference}.fa"
    output:
        "{species}/input/ref/{reference}.fa.fai"
    log:
        "{species}/input/ref/{reference}.fa.fai.log"
    params:
        extra="",
    wrapper:
