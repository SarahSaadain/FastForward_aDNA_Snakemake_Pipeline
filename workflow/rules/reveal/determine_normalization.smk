...
def combine_normalizations_for_species_input_coverage_files(wildcards):
    ...
    for individual in individuals:
        list_of_normalization_files_of_individuals.append(f"{species}/results/reveal/{feature_library}/normalization/{individual}_coverage.tsv")
...
rule determine_normalization_of_individual:
    ...
    output:
        coverage="{species}/results/reveal/{feature_library}/normalization/{individual}_coverage.tsv",
        summary="{species}/results/reveal/{feature_library}/normalization/{individual}_summary.txt"
    message: "Normalize scg and te coverage for {wildcards.individual}"
    log:
        "{species}/results/reveal/{feature_library}/normalization/{individual}_normalization.log"
    conda:
        "../../envs/python_and_r.yaml"
    script:
        "../../scripts/reveal/normalization/calculate_normalization.py"

rule combine_normalizations_for_species:
    input:
        coverage_files=combine_normalizations_for_species_input_coverage_files
    output:
        combined="{species}/results/reveal/{feature_library}/normalization/{species}_normalized_coverage.combined.tsv"
    message: "Combine normalized coverage for {wildcards.species}"
    conda:
        "../../envs/python_and_r.yaml"
    script:
        "../../scripts/reveal/normalization/combine_normalized_coverage.py"

rule plot_normalization:
    input:
        "{species}/results/reveal/{feature_library}/normalization/{species}_normalized_coverage.combined.tsv"
    output:
        directory("{species}/results/reveal/{feature_library}/normalization/plots/")
    ...
    shell:
        """
        Rscript workflow/scripts/reveal/normalization/plot_normalization.r --input "{input}" --output "{output}" --order "{params.order}" --names "{params.names}"
        """
