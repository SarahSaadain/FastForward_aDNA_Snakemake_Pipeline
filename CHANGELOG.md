...
- **`pipeline.global.skip_existing_files` now defaults to `false`** (was `true`), and pastForward logs a warning on every run in which it is enabled. When on, existing output files are dropped from the target list handed to Snakemake, so Snakemake cannot see that they are out of date: changed input files do not propagate to existing downstream outputs and results can end up inconsistent. The new default leaves Snakemake's normal re-run logic in charge. Set it to `true` explicitly to get the old behaviour, e.g. to resume an interrupted run when the inputs are known to be unchanged. `config/parameters.md`, `config/max_config_sample.yaml`, `config/config_designer.html`, and [docs/FAQ.md](docs/FAQ.md) updated to match
