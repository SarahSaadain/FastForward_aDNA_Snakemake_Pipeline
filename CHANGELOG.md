...
- **DeDup fork side-loading**: replaced the parse-time `workflow/scripts/bootstrap_dedup_jar.py` bootstrap with a `workflow/envs/dedup.post-deploy.sh` post-deploy script, so the pinned jar is side-loaded directly into the `dedup` conda environment (behind a `dedup` command matching upstream bioconda's own wrapper convention) the first time that environment is created, instead of into a repo-local `resources/dedup/` folder. If this fork's improvements are ever released under the upstream `dedup` bioconda package, the env's dependency can revert to plain `dedup` and the post-deploy script can be deleted, with no rule changes needed
